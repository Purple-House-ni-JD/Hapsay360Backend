...
export const generateStationsPdf = async (req, res) => {
    try {
        const stations = await PoliceStation.find().populate('officer_IDs', 'first_name last_name');
        ...
        const table = {
            headers: ['Custom ID', 'Name', 'Address', 'Phone', 'Landline', 'Email', 'Officers Count', 'Created At'],
            rows: stations.map(s => [
                s.custom_id || '',
                s.name || '',
                s.address || '',
